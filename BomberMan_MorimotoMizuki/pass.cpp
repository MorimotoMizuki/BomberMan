...
CPass::CPass(Point p, MapPoint system_p)
{
	CBaseEnemy::Constructor(p, system_p); //ベースのコンストラクタ

	//画像を分割
	for (int i = 0; i < PASS_IMG_NUM; i++) {
		ImgHandle[i] = DerivationGraph(IMGSIZE32 * i, IMGSIZE32 * 7, IMGSIZE32, IMGSIZE32, img);
	}

	//死亡時画像の最初の一枚の設定
	EnemyDeadImgHandle[0] = DerivationGraph(IMGSIZE32 * 4, IMGSIZE32 * 7, IMGSIZE32, IMGSIZE32, img);
...
int CPass::Action(vector<unique_ptr<BaseVector>>& base)
{
	//プレイヤーを取得
	...
	//死亡時処理
	if (IsDead) {
		CBaseEnemy::EnemyDead(PASS_ANIM_FRAME, 60);
		return 0;
	}

	//アニメーション処理
	CBaseEnemy::Anim(PASS_ANIM_FRAME, PASS_ANIM_NUM, &AnimIndex, true);
...
//敵の死亡時のパラメータ設定
void CPass::SetEnemyDeadParameter()
{
	if (IsDead) return;
