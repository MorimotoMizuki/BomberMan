...
CKondoria::CKondoria(Point p, MapPoint system_p)
{
	CBaseEnemy::Constructor(p, system_p); //ベースのコンストラクタ

	//画像を分割
	for (int i = 0; i < KONDORIA_IMG_NUM; i++) {
		ImgHandle[i] = DerivationGraph(IMGSIZE32 * i, IMGSIZE32 * 5, IMGSIZE32, IMGSIZE32, img);
	}

	//死亡時画像の最初の一枚の設定
	EnemyDeadImgHandle[0] = DerivationGraph(IMGSIZE32 * 4, IMGSIZE32 * 5, IMGSIZE32, IMGSIZE32, img);
...
int CKondoria::Action(vector<unique_ptr<BaseVector>>& base)
{
	...
	//死亡時処理
	if (IsDead) {
		CBaseEnemy::EnemyDead(KONDORIA_ANIM_FRAME, 60);
		return 0;
	}

	//アニメーション処理
	CBaseEnemy::Anim(KONDORIA_ANIM_FRAME, KONDORIA_ANIM_NUM, &AnimIndex, true);
	...
	//プレイヤー追跡処理
	TrackingPlayerMove(p, CHIP_SIZE / SPEED, &isTrackingPlayer, TrackingParameter, 10);
...
//敵の死亡時のパラメータ設定
void CKondoria::SetEnemyDeadParameter()
{
	if (IsDead) return;
